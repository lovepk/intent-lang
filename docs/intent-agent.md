...
>
> **参考实现**：`il mcp`（stdio）提供工具面——tools `il_parse` / `il_lint` / `il_diff` / `il_resolve` / `il_read` / `il_commit` / `il_log`；prompts `write` / `repro` / `accept` / `lint` / `normalize`；resources `il://spec` / `il://agent` / `il://keywords`。默认不调用模型（BYOM）。配置了 API key 时，额外提供可选编排工具 `il_record` / `il_reproduce` / `il_accept`（由服务端代调模型）。
>

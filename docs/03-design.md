...
### 6.4 分层一致性（记录优先，不干扰生成）

**最高原则：IL 是记录介质，不是控制手段。** 系统只负责产出"合法且自洽的记录"，不提示、不否决、不扣留、不阻断生成。一致性靠**确定性构造 + 分层检验**保证，而非靠拒绝。

| 层 | 检查什么 | 谁做 | 能否保证 | 触发 |
|---|---|---|---|---|
| L0 生成 | 不检查、不干扰 | 生成器（LLM 自由产出草稿） | — | 每轮 |
| L1 确定性 | 结构合法（parse/validate）+ **机器 diff**（实际改了哪些单元）+ 跨段自洽（lint error） | 机器（纯计算） | 能 | 每轮 |
| L2 语义 | L1 修不了的非法/矛盾 | 记录员（**同模型、不同角色**：`ModeNormalize`/`NormalizePrompt`） | 概率 | 仅 L1 报警时 |
| L3 行为 | 档案 ↔ 产物 | ACCEPT 验收 | 对枚举用例能 | 按需 |
| L4 人 | 最终裁决 | 人 | — | 随时 |

- **L1 是记录地基**：机器 diff 保留，作为"本次实际改了什么"的权威记录；结构校验与 lint 同层，确定性、零成本、每轮都跑。变更单元统一由 `il.EntryBodies` 定义（`R/A/D/?`、头字段 `INTENT/KIND/FIDELITY/TARGET`、`ANCHORS`、`SNIPPET:<label>`），`archive.Summarize` 复用同一 diff，保证记录与摘要口径一致。
- **L2 只在 L1 报警时出手**：记录员把草稿整理成合法自洽，**不改事实、不拒绝、不提示**；若记录员也救不回，按草稿记录（只记录，不阻断）。不采用每轮两遍。
- **reply 只是记录**：权威意图只有档案，reply 作为"模型的话"原样记录，不做一致性判定，也不因不一致而阻断。机器 diff 作为权威变更面板独立展示。
- **层不越权**：L1 不猜语义，L2 不碰确定性事实，L4 才拍板。
- **已移除**：变更闸门、`declared_changes`、校验拒绝、reply 扣留、所有 in-chat 提示——它们都属"干扰"。
- 实现：`internal/il/diff.go`（EntryBodies/CompareEntries）、`cmd/il/normalize.go`（l1Problems/recordArchive）、`provider.ModeNormalize`。

> 设计沿革：早期曾用"变更闸门 + `declared_changes`"做声明-校验式阻断，并用 `CheckReplyClaims`/`lint --llm --reply` 做 reply↔档案提示。实测（8 场景）显示模型对"穷举声明"遵守度不足、拒绝过多，且阻断本身违反"记录不干扰"原则，故整体替换为上述分层记录模型。
